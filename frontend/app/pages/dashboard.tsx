...
export default function Dashboard() {
  ...
  const fetchLoans = async () => {
    try {
      const response = await axios.get<Loan[]>("http://localhost:5000/api/loans")
  ...
  const handleUpdate = async (loan: Loan) => {
    try {
      await axios.put(`http://localhost:5000/api/loans/${loan._id}`, loan)
      toaster.create({
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-extrabold text-gray-900 mb-10 text-center">Loan Management System</h1>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <LoanForm onLoanAdded={fetchLoans} editingLoan={editingLoan} onUpdate={handleUpdate} />
            </div>
          </div>
